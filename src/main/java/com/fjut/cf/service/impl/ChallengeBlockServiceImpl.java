...
public class ChallengeBlockServiceImpl implements ChallengeBlockService {
    ...
    public List<UserChallengeBlockVO> queryUserChallengeBlockByUsername(String username) {
        ...
        // 取得用户的全部挑战模块
        List<UserChallengeBlockVO> allBlocks = challengeBlockMapper.queryAllChallengeBlockVO();
        // 将结果集放入map中
        ...
        // 获取用户已经解锁的模块ID
        List<Integer> openBlocks = challengeUserOpenBlockMapper.queryChallengeUserOpenBlockByUsername(username);
        ...
        // 获取模块的得分记录
        List<UserChallengeBlockVO> blockScoredS = challengeBlockProblemMapper.queryChallengeBlockScoredByUsername(username);
        ...
        // 获取用户可以展示的结果集的ID集合
        List<Integer> showedIds = challengeBlockMapper.queryChallengeBlockShowedByUsername(username);
    ...
    @Override
    public ChallengeBlockVO queryChallengeBlockByBlockIdAndUsername(Integer blockId, String username) {
        ChallengeBlockVO result = new ChallengeBlockVO();
        ChallengeBlockPO challengeBlockPO = challengeBlockMapper.queryChallengeBlockByBlockId(blockId);
        ...
        // TODO
        Integer totalScore = challengeBlockProblemMapper.queryBlockTotalScoreByBlockId(blockId);
        result.setTotalScore(totalScore);
        Integer getScore = challengeBlockProblemMapper.queryBlockScoredByBlockIdAndUsername(blockId, username);
        result.setGetScore(getScore);
        return result;
    }

    @Override
    public List<ChallengeBlockConditionPO> queryAllChallengeBlockCondition() {
        return challengeBlockConditionMapper.queryAllChallengeBlockCondition();
    }

    @Override
    public List<ChallengeBlockConditionVO> queryChallengeBlockConditionByBlockId(Integer blockId) {
        return challengeBlockConditionMapper.queryChallengeBlockConditionByBlockId(blockId);
    }
    ...
    public void updateOpenBlock(String username, Integer problemId) {
        ...
        Integer blockId = challengeBlockProblemMapper.queryChallengeBlockByProblemId(problemId);
        // 如果题目不属于任何挑战模式模块，则返回
        if (null == blockId) {
            return;
        }
        // 获取题目隶属挑战模块的后置模块
        List<Integer> belongBlocks = challengeBlockConditionMapper.queryRearBlocksByBlockId(blockId);
        // 遍历这些后置模块，依次取出他们的各个条件，逐一判断是否满足
        for (Integer belongBlock : belongBlocks) {
            boolean canOpen = true;
            // 取出解锁条件
            List<ChallengeBlockConditionVO> conditionVOS = challengeBlockConditionMapper.queryChallengeBlockConditionByBlockId(belongBlock);
            // 遍历条件看是否全部满足，如果有部分不满足则标识位 canOpen = false
            for (ChallengeBlockConditionVO conditionBlock : conditionVOS) {
                // 取得模块获得分数
                Integer score = challengeBlockProblemMapper.queryBlockScoredByBlockIdAndUsername(conditionBlock.getBlockId(), username);
            ...
            if (canOpen) {
                ...
                challengeUserOpenBlockMapper.insertChallengeUserOpenBlock(userOpenBlockPO);
            }
        }
